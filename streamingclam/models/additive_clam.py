...
from streamingclam.models.additive_mil import AdditiveClassifier
...
class CLAM_SB(nn.Module):
    def __init__(self, gate=True, size=[1024, 512, 256], dropout=False, k_sample=8, n_classes=2,
        ...
        if self.additive:
            self.classifiers = AdditiveClassifier(input_dims = size[1], output_dims=n_classes, hidden_dims=[])
        else:
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        if self.additive:
            classifier_out_dict = self.classifiers(h, A.T)
            logits = classifier_out_dict["logits"]
        else:
            M = torch.mm(A, h)
        ...
        if instance_eval:
            ...
            results_dict = {"patch_logits": classifier_out_dict["patch_logits"]}
        else:
            results_dict = {}
        if return_features:
            results_dict.update({'features': torch.mm(A, h)})
